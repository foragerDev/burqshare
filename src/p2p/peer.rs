...
impl LocalPeer {
    pub async fn announce(name: &str) {
        ...
        socket.join_multicast_v4(&MULTICASE_ADD, &Ipv4Addr::UNSPECIFIED);
        let mut message = PeerMessage::Connecting;
        let mut buffer = [0u8; 256];
        while let PeerMessage::Connecting = message {
            socket.send_to(serde_json::to_string(&peer).unwrap().as_bytes(), multicase_socket);
            let bytes = socket.recv_from(&mut buffer).unwrap();
